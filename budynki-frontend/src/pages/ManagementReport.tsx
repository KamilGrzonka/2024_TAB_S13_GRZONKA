...
//     : undefined;
  ...
  const buildingsProfit = useQuery({
    ...
    queryFn: () => getBackendApi<BuildingProfit[]>(`/budynki/zyski?dataPoczatkowa=${dataPoczatkowaParam}&dataKoncowa=${dataKoncowaParam}`),
  });
            ...
            {buildingsProfit.data.map((buildingProfit) => (
              <TableRow key={buildingProfit.budynekId}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                    {buildingProfit.wartosc > 0 ? <MoveUp className="text-green-500" /> : buildingProfit.wartosc < 0 ? <MoveDown className="text-red-500" /> : <>-</>}
                </div>
                </TableCell>
              </TableRow>
            ))}
